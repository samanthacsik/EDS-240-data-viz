...
[1. labels -- add titles, axis / legend labels]{.body-text-s}     
...
[{{< fa chart-line title="a line chart" >}} *NOTE: Omitting argument names*]{.slide-title}
...
[{{< fa chart-line title="a line chart" >}} *NOTE: Defining data & mappings in `geom_*()`*]{.slide-title2}
...
[{{< fa palette title="a painter's palette" >}} *NOTE: Mapping color at a local level*]{.slide-title}
...
[{{< fa palette title="a painter's palette" >}} *NOTE: Why map locally?*]{.slide-title}
...
[{{< fa palette title="a painter's palette" >}} *NOTE:* What if we just want to color all points the same?]{.slide-title3}
...
[{{< fa palette title="a painter's palette" >}} *NOTE:* We can also map our own colors]{.slide-title}
...
## {#pos-default data-menu-title="Default position geom_bar()"} 

[{{< fa chart-line title="a line chart" >}} The default position for `geom_bar()` is "stack"]{.slide-title3}

<hr>

**Every geom has a default position.**

The default position used in `geom_bar()` is **stack**, which stacks bars on top of one another, based on the `fill` value (here, that's `island`):
```{r}
#| eval: true
#| echo: true
#| code-line-numbers: "5"
...
**By default, ggplots are constructed in a Cartesian coordinate system**, consisting of a horizontal x-axis and vertical y-axis.
