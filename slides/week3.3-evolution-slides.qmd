...
The aspect ratio is the **height:width ratio of a graph**. For line graphs in particular, aspect ratio affects the perceived steepness of a sloped line. The **larger the aspect ratio, the steeper changes appear**, which may cause readers to **interpret changes as more important**. The **smaller the aspect ratio, the flatter the line** which may cause readers to **interpret changes as small / insignificant**.
...
## {#aspect-ratio-ggplot data-menu-title="Adjust aspect ratio"} 

[Adjust the aspect ratio using `coord_fixed()`]{.slide-title2}

<hr>

[There's **no exact rule** for what aspect ratio to use for a given graphic (but see [Cleveland et al. 1988](https://www.jstor.org/stable/2288843) to read about the "banking to 45 degrees" rule) -- it depends on the nature of the variable and your goal with the visualization. *But* it's important to keep mind that manipulating the aspect ratio can mislead readers, and so you should do so with carefully and intentionally.]{.body-text-s}
...
ntl_airtemp |> 
  ...
  ggplot(aes(x = sampledate, y = ave_air_temp_adjusted)) +
  geom_line() + 
  coord_fixed(ratio = 1)

arc_weather |> 
  filter(date > "2017-12-31") |> 
  ggplot(aes(x = date, y = mean_airtemp)) +
  geom_line() +
  coord_fixed(ratio = 5)
```
