...
[What do we mean by visualizing data "distributions"?]{.slide-title3}
...
[Plotting all possible values or intervals of data on a frequency graph.]{.body-text-l}
...
[Plotting summary statistics of all values indicating their spread.]{.body-text-l}
...
Overlaying a histogram and density plot requires scaling down the histogram to match the density curve scale. Do so adding the `y = ..density..` within the `aes()` function:
```{r}
#| eval: true
#| echo: true
...
Highlight a group of interest by first creating a column in the data frame indicating which groups to highlight
```{r}
#| eval: true
#| echo: true
