...
`{RColorBrewer}` comes with a couple useful functions for quickly viewing and assembling your palette HEX codes: 
...
[Use paletteer to access TONS of palettes]{.slide-title2}

<hr>

[It would be impossible to list out on one slide all the palette packages available for use, but the [`{paletteer}` package](https://emilhvitfeldt.github.io/paletteer/) gets close to doing just that -- it provides **a common interface for accessing a near comprehensive list of palettes (over 2,000!!) across various packages**. A couple options:]{.body-text-s}

- [Use it to **generate a list of HEX codes** from any of the supported palette packages using one of [three functions](https://emilhvitfeldt.github.io/paletteer/#examples): `paletteer_c()`, `paletteer_d()`, `paletteer_dynamic()`]{.body-text-s}
- [Use it to **apply a palette directly to your ggplot** using one of [three scaling functions](): `scale_*_paletteer_c()`, `scale_*_paletteer_d()`, `scale_*_paletteer_binned()`]{.body-text-s}
...
[Find public access to a massive inventory of climate data on their [Climate Monitoring](https://www.ncei.noaa.gov/access/monitoring/products/) page. Today's lesson will use the [Climate at a Glance](https://www.ncei.noaa.gov/access/monitoring/climate-at-a-glance/) collection. Specifically, **we'll be exploring county-level precipitation data from October 2023, accessed via the [County Mapping](https://www.ncei.noaa.gov/access/monitoring/climate-at-a-glance/county/mapping/110/pcp/202310/1/value) portal**.]{.body-text-s}
...
precip_counties <- read_csv(here::here("slides", "data", "county-precip-2023.csv"), skip = 4) |> 
  janitor::clean_names()
...
#....................import precipitation data...................
precip_counties <- read_csv(here::here("week5", "data", "county-precip-2023.csv"), skip = 4) |> 
  janitor::clean_names()
...
## {#consider-data data-menu-title="Consider data & colors"} 

[Let's first consider our data]{.slide-title}

<hr>

There are pros and cons to both classed and unclassed color scales.

Use a **classed** if we want to **communicate statistical brackets**:
  - the focus is on which data units fall into a particular predefined class, rather than the overall pattern of the map
  - If you want your readers to see if certain regions fall into a statistical bracket, use classes
  - Use a classed color scales if you want readers to read values
  - more classes, less readable
  - So if you want readers to read specific value ranges, go for a classed map with only a few classes. That’s especially important when you show static maps (e.g., in print or PDF reports) where people don’t have the option to see tooltips or hover over a legend.
  
Use an **unclassed** color scale if you want to **show a general pattern** (e.g. "the west got less precipitation than the east")
  - makes it harder to see which statistical brackets counties fall into (e.g. which counties got more or less precipitation than the national average)
  - “The unclassed choropleth is the most exact representation of the data model possible,” Judith A. Tyner
  - The more classes you show, the more nuanced your map becomes
  - Use an unclassed color scale if you don’t want to interpret for the reader. It makes it easier to see:
    - …outliers
    - …if the transition between clusters is smooth or abrupt
    - …if the region I’m interested in as a reader has a higher or lower value than the neighboring regions
  - “How does my region compare?” is a question that’s better answered with an unclassed map.
...
## {#colorbar-styling-fxn data-menu-title="Colorbar styling function"} 

[Create a function for adding the correct colorbar styling]{.slide-title3}

<hr>

[We're going to create a couple different choropleths to look at the differences between gradient vs. binned color scales. Customizing the appearance (title, position, width / height) of our legend requires two different functions (`guide_colorbar()` and `guide_colorsteps()`), depending on which type of scale you use. ]{.body-text-s}

[Instead of having to type these long functions out each time we want to style our colorbar, we can instead **write our own function to streamline our code**:]{.body-text-s}
...
## {#initial-attempts data-menu-title="Initial mapping attempts)"} 

[A few initial mapping attempts]{.slide-title}
...
my_brew_palette <- RColorBrewer::brewer.pal(n = 11, name = 'BrBG')
```
...
::: {.footer}
By default, `scale_fill_stepsn()` sets `nice.breaks = TRUE`
:::
...
```{r}
mean_precip <- mean(na.omit(precip_counties_wrangled$precip)) 
mean_precip # 36.5

total_precip <- sum(na.omit(precip_counties_wrangled$precip))
num_counties <- length(unique(na.omit(precip_counties_wrangled$county)))

total_precip / num_counties
total_precip / 3107
```
