...
top_3_coo_names <- top_3_coo |> #top_3_coo$coo_name 
  pull(coo_name)

# top three countries of asylum ----
top_3_coa <- pop2022 |> 
  group_by(coa_name) |> 
  summarize(num_by_coa = sum(refugees)) |> 
  slice_max(order_by = num_by_coa, n = 3)

top_3_coa_names <-top_3_coa |>  #top_3_coa$coa_name
  pull(coa_name)

# 'other' cat for country of origins not in top 3 ----
pop2022 <- pop2022 |> 
  mutate(coo_name = as.factor(coo_name),
         coa_name = as.factor(coa_name)) |> 
...
# pop2022_no_other$IDsource = match(pop2022_no_other$coo_name, nodes$name) - 1
# pop2022_no_other$IDtarget = match(pop2022_no_other$coa_name,  nodes$name) - 1

# color scale ----
ColorScale = 'd3.scaleOrdinal([`#94438B`, `#B6366A`,`#E1AAC9`, `#909497`,`#6AA26F`, `#A3C7AC`, `#E3EEAE`])' # `#a36629`, `#92462f`
