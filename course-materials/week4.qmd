---
title: "Fundamental Chart Types (Part II)"
subtitle: "Week 4"
description: "Monday January 29^th^, 2024"
title-block-banner: true
toc: true
---

```{r}
#| eval: true 
#| echo: false
#| fig-align: "center"
#| out-width: "65%" 
#| fig-alt: "A group of fuzzy round monsters with binoculars, backpacks and guide books looking up a graphs flying around with wings (like birders, but with exploratory data visualizations). Stylized text reads 'ggplot2: visual data exploration.' Learn more about ggplot2."
knitr::include_graphics("../images/horst-ggplot-explore-dataviz.png")
```

:::{.gray-text .center-text}
*Artwork by [Allison Horst](https://allisonhorst.com/)*
:::

## Pre-class Prep

*Coming soon* 

## Lecture Materials 

Week 4 instruction is broken down into three lessons:

::: {.grid}

::: {.g-col-12 .g-col-md-4}
::: {.center-text}
[**Visualizing correlations**]{.teal-text .body-text-m}

[{{< fa person-chalkboard title="A person motioning at a chalkboard" >}} lecture 4.1 slides](../slides/week4.1-correlations-slides.qmd){.btn role="button" target="_blank"} 
:::
:::

::: {.g-col-12 .g-col-md-4}
::: {.center-text}
[**Visualizing rankings**]{.teal-text .body-text-m}

[{{< fa person-chalkboard title="A person motioning at a chalkboard" >}} lecture 4.2 slides](../slides/tbd.qmd){.btn role="button" target="_blank"} 
:::
:::

::: {.g-col-12 .g-col-md-4}
::: {.center-text}
[**Miscellaneous chart types**]{.teal-text .body-text-m}

[{{< fa person-chalkboard title="A person motioning at a chalkboard" >}} lecture 4.3 slides](../slides/tbd.qmd){.btn role="button" target="_blank"} 
:::
:::

:::

## Discussion Materials

*Coming soon!*

<!-- Critiquing data viz? -->
<!-- see: https://samanthacsik.github.io/EDS-240-data-viz/slides/week3.1-choosing-graphic-forms-slides.html#/critique-considerations -->

## Assignment Reminders

| Assignment Type | Assignment Title        | Date Assigned  | Date Due                   |
|:---------------:|-------------------------|----------------|----------------------------|
| EOC             | EOC (week 4)            | Mon 01/29/2024 | Mon 01/29/2024, 11:55pm PT |
| HW              | Homework Assignment #2  | Mon 01/22/2024 | Sat 02/03/2024, 11:59pm PT |

<!-- | Assignment Type | Assignment Title                                                       | Date Assigned  | Date Due                   | -->
<!-- |:---------------:|------------------------------------------------------------------------|----------------|----------------------------| -->
<!-- | EOC             | [EOC (week 4)](https://forms.gle/kwfaWciZNu4fCzKN7)                    | Mon 01/29/2024 | Mon 01/29/2024, 11:55pm PT | -->
<!-- | HW              | [Homework Assignment #2](../assignments/HW2.qmd)                       | Mon 01/22/2024 | Sat 02/03/2024, 11:59pm PT | -->

