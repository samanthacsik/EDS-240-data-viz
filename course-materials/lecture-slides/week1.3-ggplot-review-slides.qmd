...
[Week 1 | January 6^th^, 2024]{.custom-subtitle3}
...
Take this [tibble](https://tibble.tidyverse.org/) (very similar to a `data.frame`) of temperature recordings at three stations on three dates:
...
Next, we'll layer on a geometric object (aka geom) that our plot will use to represent our penguin data. There are many geoms (`geom_*()`) that are [built into `{ggplot2}` already](https://ggplot2.tidyverse.org/reference/#geoms) (and more when you use extension packages). To create a scatterplot:
```{r}
#| eval: true
#| echo: true
#| code-line-numbers: "2"
...
penguins |>
  ...
  geom_point(color = "peachpuff4") # including simply bc I love this recognized color name
```
